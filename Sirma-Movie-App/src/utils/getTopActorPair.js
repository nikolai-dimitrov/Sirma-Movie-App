export const getTopActorPair = (moviesByIds, rolesByIds) => {
	...
	for (const id in moviesByIds) {
		const movie = moviesByIds[id];
		...
		if (roles) {
			roles.sort(
				(roleA, roleB) =>
					rolesByIds[roleA].ActorID - rolesByIds[roleB].ActorID
			);
			...
			for (let i = 0; i < roles.length - 1; i++) {
				...
				const currentActorId = rolesByIds[currentRoleId].ActorID;

				for (let j = i + 1; j < roles.length; j++) {
					const nextRoleId = roles[j];
					const nextActorId = rolesByIds[nextRoleId].ActorID;
